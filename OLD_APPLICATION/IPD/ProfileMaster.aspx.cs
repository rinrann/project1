...
public partial class Pathology_ProfileMaster : System.Web.UI.Page
{
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        {
            theprofile.InsertProfile(txtcode.Text, txtname.Text, txtprice.Text, DropDownList1.SelectedValue, Session["CoCode"].ToString(), Session["userName"].ToString());
            for (int i = 1; i < GridView2.Rows.Count; i++)
            {
                Label lblid = (Label)GridView2.Rows[i].Cells[2].FindControl("lblid");

                theprofile.InsertMapping(HiddenField1.Value, lblid.Text, Session["CoCode"].ToString(), Session["userName"].ToString());
            }
            ScriptManager.RegisterStartupScript(this, this.GetType(), "msg", "alert('Inserted Successfully !');", true);
        }
        else
        {
            theprofile.DeleteMapping(HiddenField1.Value, txtcode.Text, Session["CoCode"].ToString());
            theprofile.UpdateProfile(txtcode.Text, txtname.Text, txtprice.Text, DropDownList1.SelectedValue, Session["CoCode"].ToString(), Session["userName"].ToString());
