...
public partial class DayCare_AppoinmentList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Objmain.checkaccss(Session["CoCode"].ToString(), Session["UserRoleID"].ToString().Trim(), "APPOINTMENT LIST", checkAccessType.ViewAction) == false)
        {
        ...
        if (!IsPostBack)
        {
           // txtdate.Text = DateTime.Now.ToString("dd/MM/yyyy");
            ...
            if (Request.QueryString["Date"] != null || Request.QueryString["shift"] != null || Request.QueryString["Title"] != null || Request.QueryString["NoOfPatient"] != null)
            {
                ...
                //string dt1 = DateTime.ParseExact(dt, "dd/MM/yyyy", dtf).ToString("yyyyMMdd");
                string dt1 = Convert.ToDateTime(dt).ToString("yyyy-MM-dd");
                //string dt1 = Convert.ToDateTime(dt).ToString("dd/MM/yyyy");
                //DateTime dateonly = dt1;
                //code by sankar
                //string[] tdate1 = dt1.Split('%');
                //string tdate2=tdate1[0];
                //DateTime testdate = DateTime.ParseExact(dt, "dd/MM/yyyy", dtf);
                //DateTime dateOnly = testdate.Date;
                //string date = dateOnly.ToString();
                //string testdate1 = DateTime.ParseExact(dt, "dd/MM/yyyy", dtf).ToString("dd/MM/yyyy");
                txtdate.Text = dt1;
    ...
    private void GridFill()
    {
        ...
        if (txtdate.Text != "")
        {
             //testdate = Convert.ToDateTime(txtdate.Text).ToString("yyyy/MM/dd");
             //testdate = txtdate.Text;
            // tdate = testdate.Substring(0, 4) + "/" + testdate.Substring(5, 2) + "/" + testdate.Substring(8, 2);
            ////tdate = txtdate.Text.Substring(0, 4) + "-" + txtdate.Text.Substring(5, 2) + "-" + txtdate.Text.Substring(8, 2);
            ////testdate = DateTime.ParseExact(date3, "yyyy-MM-dd", dtf).ToString("yyyy-MM-dd");
            /////string testdate = DateTime.ParseExact(testdate, "yyyy-MM-dd", dtf).ToString("dd/MM/yyyy");
        }
        else
        {
           // tdate = "";
        ...
        //lblError.Text = testdate;
        GridView1.DataSource = thepatientAppo.GridAppoList(Session["CoCode"].ToString().Trim(), Session["YearCode"].ToString().Trim(), DropDownList1.SelectedValue.ToString(),txtdate.Text  , txtname.Text.Trim(), txtreg.Text.Trim());
        //GridView1.DataSource = thepatientAppo.GridAppoList(Session["CoCode"].ToString().Trim(), Session["YearCode"].ToString().Trim(), DropDownList1.SelectedValue.ToString(), "2016/04/27", txtname.Text.Trim(), txtreg.Text.Trim());
        GridView1.DataBind();
    }
