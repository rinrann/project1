...
public partial class DayCare_DisposalMapping : System.Web.UI.Page
{
    Main Objmain = new Main(ConfigurationSettings.AppSettings["ConnectionString"].ToString());
    DC_DisposalMapping thedisp = new DC_DisposalMapping(ConfigurationSettings.AppSettings["ConnectionString"].ToString());
    protected void Page_Load(object sender, EventArgs e)
    {
        Page.Title = "Disposable Mapping";
        if (Session["userName"] == null)
        {
            Response.Redirect("../LoginPage.aspx");
        }
        if (Objmain.checkaccss(Session["CoCode"].ToString(), Session["UserRoleID"].ToString().Trim(), "DISPOSABLE MAPPING", checkAccessType.ViewAction) == false)
        {
            Response.Redirect("../AccessDenied.aspx");
        }

        if (Objmain.checkaccss(Session["CoCode"].ToString(), Session["UserRoleID"].ToString().Trim(), "DISPOSABLE MAPPING", checkAccessType.InsertAction) == false)
        {
    ...
    {
        this.ddldialysertype.DataSource = thedisp.DropdownDialysertype();
    ...
    {
         this.ddldialysername.DataSource = thedisp.DropdownDialyserName(Convert.ToInt32(ddldialysertype.SelectedValue));
    ...
    {
        thedisp.DeleteMapping(ddldialysertype.SelectedValue, ddldialysername.SelectedValue, Session["CoCode"].ToString());
    ...
    {
        GridView1.DataSource = thedisp.GridDisposalDetails(ddldialysertype.SelectedValue.ToString(), ddldialysername.SelectedValue.ToString());
        GridView1.DataBind();
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DataTable dt = thedisp.GridDisposalDatabound(ddldialysertype.SelectedValue, ddldialysername.SelectedValue);
            if (dt.Rows.Count > 0)
            {
