...
public partial class Pathology_TestPopup : System.Web.UI.Page
{
    ...
    {
        if (Objmain.checkaccss(Session["CoCode"].ToString(), Session["UserRoleID"].ToString().Trim(), "TEST MASTER", checkAccessType.DeleteAction) == false)
        {
            GridView_popup.Columns[5].Visible = false;
        }
        if (!IsPostBack)
        {
            DropDownFill();
